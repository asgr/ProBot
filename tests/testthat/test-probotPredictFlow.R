...
test_that("probotMarginalPostNF returns mean/sd matrices with names", {
  mdl <- sigma_test_flow()
  X <- matrix(rnorm(20 * 2), 20, 2)
  r <- probotMarginalPostNF(X, mdl, output_dim = 3,
                         col_names = c("A", "B", "C"))
  ...
  r1 <- probotMarginalPostNF(X[1, ], mdl, output_dim = 3, col_names = c("A", "B", "C"))
...
test_that("probotMarginalPostNF sd is exactly the Jacobian row norm", {
  ...
  r <- probotMarginalPostNF(X, mdl, output_dim = D, eps = eps, batch_size = 4)
...
test_that("probotMarginalPostNF centre matches point_estimate = TRUE", {
...
test_that("probotMarginalPostNF sd agrees with sampling for a learnable problem", {
  ...
  for (style in c("realnvp", "maf")) {
    ...
    r <- probotMarginalPostNF(te, mdl, output_dim = 3, batch_size = 30)
...
test_that("probotMarginalPostNF unscales mean and sd consistently", {
  mdl <- sigma_test_flow()
  X <- matrix(rnorm(10 * 2), 10, 2)
  cm <- c(10, 20, 30); cs <- c(2, 3, 4)
  raw <- probotMarginalPostNF(X, mdl, output_dim = 3)
  uns <- probotMarginalPostNF(X, mdl, col_means = cm, col_sds = cs)
  ...
  one <- probotMarginalPostNF(X, mdl, col_means = 5, col_sds = 2, output_dim = 3)
  expect_equal(one$post_sd, raw$post_sd * 2, tolerance = 1e-4)
  expect_equal(one$post_mean, raw$post_mean * 2 + 5, tolerance = 1e-4)
})

test_that("probotMarginalPostNF is deterministic in batch_size", {
  # The whole point of a deterministic grid: unlike the sampler, chunking
  # cannot change the answer.
  mdl <- sigma_test_flow()
  X <- matrix(rnorm(12 * 2), 12, 2)
  a <- probotMarginalPostNF(X, mdl, output_dim = 3, batch_size = 1)
  b <- probotMarginalPostNF(X, mdl, output_dim = 3, batch_size = 12)
  expect_equal(a$post_sd, b$post_sd, tolerance = 1e-4)
  expect_equal(a$post_mean, b$post_mean, tolerance = 1e-4)
})

test_that("probotMarginalPostNF clamps an over-budget batch_size with a warning", {
  output_dim <- 3
  mdl <- mock_flow(output_dim, act = 64000L)  # row budget of exactly 1000
  ...
  expect_warning(
    r <- probotMarginalPostNF(input, mdl, output_dim = output_dim, batch_size = 200),
  ...
  r2 <- probotMarginalPostNF(input, mdl, output_dim = output_dim, batch_size = 3)
  expect_equal(r2$post_sd, r$post_sd, tolerance = 1e-6)
})

test_that("probotMarginalPostNF errors on the same inputs as the sampler", {
  mdl <- sigma_test_flow()
  X <- matrix(rnorm(4 * 2), 4, 2)
  expect_error(probotMarginalPostNF(X, mdl), "Either 'output_dim' or 'col_means'")
  expect_error(probotMarginalPostNF(X, mdl, col_means = c(1, 2, 3)),
               "col_sds must be provided")
  expect_error(probotMarginalPostNF(X, mdl, output_dim = 3, eps = 0),
               "'eps' must be a single positive number")
  expect_error(probotMarginalPostNF(X, mdl, output_dim = 3, eps = NA_real_),
               "'eps' must be a single positive number")
  expect_error(probotMarginalPostNF(array(rnorm(8), c(2, 2, 2)), mdl, output_dim = 3),
               "must be either a vector or a matrix")
})

test_that("probotMarginalPostNF works for a location-head flow", {
