...
vignette: >
  %\VignetteIndexEntry{ProBot: Back and Forth}
...
We build an MDN with a three-layer encoder (64 → 128 → 64 hidden units) and three Gaussian mixture components. Five components give the network enough flexibility to represent multimodal or skewed predictive distributions without being unnecessarily large.
...
forward_optimizer <- optim_adam(forward_model$parameters, lr = 3e-4)
...
```{r fig.width=6, fig.height=6, eval=eval_global}
magtri(post_samples, refvals = poly_test$output[index,],
...
for(i in 1:9){
  sel = 1:1e4
  magplot(poly_test$output[sel,i], forward_summary$post_mean[sel,i],
          xlab=paste0('Truth output: ',i), ylab=paste0('Pred output: ',i), pch='.')
  magerr(poly_test$output[sel,i], forward_summary$post_mean[sel,i],
         ylo = forward_summary$post_sd[sel,i], col=hsv(v=0, alpha=0.05))
}
```
...
inverse_optimizer <- optim_adam(inverse_model$parameters, lr = 3e-4)
