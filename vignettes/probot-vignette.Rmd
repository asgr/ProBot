...
The `lambda` parameter is an L2 regularisation weight applied to the network parameters to discourage overfitting. `early_stop = FALSE` runs for the full 100 epochs here; in practice you would set `early_stop = TRUE` and tune `stop_window` and `stop_delta` so that training halts automatically once the loss has plateaued.
...
A monotonically decreasing loss that flattens toward the end of training is healthy. If the loss stops decreasing early, try a lower learning rate or more epochs. If it decreases but then increases (overfitting), reduce the network size, increase `lambda`, or enable early stopping.
...
The triangle (corner) plot below shows the pairwise marginal distributions of the nine polynomial outputs for the first test observation. The red reference lines mark the true values. Well-calibrated samples should scatter symmetrically around the truth.
